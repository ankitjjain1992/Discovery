...
public class MyVideosPage {
	...
	/*
	 * @author Ankit Jain verify given program is present in the video you like
	 * ribbon
	 */
	public void isProgramPresent(String programName) throws Exception {
		List<WebElement> AllLikedProgram = webdriver.findElement(By.xpath(videosULikeRibbonXpath))
				.findElements(By.xpath(programNameXpath));
		...
		while (true) {
			...
			if (count <= 20 && ispresent == false) {
				webdriver.findElement(By.xpath(videosULikeRibbonXpath)).findElement(By.xpath(rightarrowRibbonXpath))
						.click();
			}
			count++;
		}
